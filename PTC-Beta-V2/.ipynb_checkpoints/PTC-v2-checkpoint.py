...
def visualize_entry(df, pattern: List[Tuple], symbol: str, tf: int, order_type: int, entry: float) -> None :
    ...
    order = "buy order" if order_type == 2 else "sell order"
    fig.write_image(f"{symbol} {order} at {entry}.jpeg")
...
def chuks_type1_buy_model(symbol, timeframe, function_dict, tag="1B"):
    ...
    rd = abs(Decimal(str(tick_size)).as_tuple().exponent)
    ...
    for pattern in search_patterns(all_swings, df, type1_buy_pattern, tag, indexes, function_dict):
        ...
        if prices not in symbols[symbol]:
            symbols[symbol].append(prices)
            ...
            risk = 79 * 0.1  # 10% of account || should be 10% of starting capital
            ...
            lot = max(round(risk/(tick_value * n_ticks), 3), symbol_info.volume_min)

            actual_risk = lot * tick_value * n_ticks

            if actual_risk < risk:
                sl_ticks = (risk * tick_size) / (lot * tick_value)
                sl = round(entry_price - sl_ticks, rd)

            tp = max(round(entry_price + (3 * sl_ticks) + spread, rd), round(prices[5] + spread, rd))
            
            try:
                visualize_entry(df, pattern, symbol, timeframe, order, entry_price)
            except Exception as e:
...
def chuks_type1_sell_model(symbol, timeframe, function_dict, tag="1S"):
    ...
    for pattern in search_patterns(all_swings, df, type1_sell_pattern, tag, indexes, function_dict):
        ...
        if prices not in symbols[symbol]:
            ...
            risk = 79 * 0.1  # 10% of account   
            
            sl = round(prices[2] + (1.5 * atr) - spread, rd)
            ...
            lot = max(round(risk/(tick_value * n_ticks), 2), symbol_info.volume_min)
            ...
            tp = min(round(entry_price - (3 * sl_ticks) - spread, rd), round(prices[5] - spread, rd))
